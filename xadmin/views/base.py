...
from base.models import UserProfile  # PSM add for BU check
...
class CommAdminView(BaseAdminView):
    ...
    site_title = getattr(settings, "XADMIN_TITLE", _(u"Thinkin Xadmin"))
    site_footer = getattr(settings, "XADMIN_FOOTER_TITLE", _(u"Thinkin Co."))
    ...
    def get_context(self):
        ...
        context.update({
            ...
            'nav_menu': nav_menu,
            'account_bu': account_bu,   # PSM add for BU check
...
# PSM add to get user-account BU setting as global variables
def get_account_bu(self):
    #
    userid = self.request.user.id
    am = UserProfile.objects.filter(id=userid)
    if am:
        ...
        account_bu = ''
    #
    if self.request.user.is_superuser and account_bu == '':
        account_bu = '*ALL'
    else:
        pass
    return account_bu